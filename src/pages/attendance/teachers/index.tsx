...
export const HistorySheet = ({ show, setShow, selectedId }: any) => {
  ...
  const getData = async () => {
    setLoading(true);
    try {
      const response: any = await attendanceServices.getTeacherAttendancesLog(selectedId);
      setData(response);
    } catch (error) {
                          ...
                          <p
                            ...
                            onClick={() => {
                              if (log?.action == 4) {
                                ...
                                navigate(`/attendance/teachers/detail-compare/${selectedId}/log/${log?.id}`);
                              }
                            }}
                          >
...
const AttendanceTeachersList = (props: Props) => {
  const { t } = useT();
  const navigate = useNavigate();
  ...
  const handleDelete = async () => {
    ...
    try {
      setLoading(false);
      const response: any = await attendanceServices.deleteTeacherAttendance(Number(selectedId));
  ...
  const getData = async (filters?: any) => {
    let params: any = {
      fields:
        'identity,timeline_id,attendance_code,classroom_id,status,teaching_program_id,teaching_program_detail_number,admin_assigned_id,subject_id,location_id,teacher_id,start_time,end_time,checkin_timestamp,checkout_timestamp,note,valid,created_at,updated_at,teaching_program_detail,teaching_program,teacher_id,user_ability',
      limit,
      offset,
      filterings: {}
    };
    if (infoSearch?.teacher_attendance_code) {
      params.filterings['teacher_attendance_code:contain'] = infoSearch?.teacher_attendance_code;
    }
    ...
    setLoading(true);
    try {
      const response: any = await attendanceServices.getTeacherAttendances(params);
        ...
        <div className='w-full overflow-hidden rounded-3xl border border-violet-200 bg-white shadow-sm'>
          <div className='border-b border-violet-200 bg-violet-50 px-5 py-5 text-slate-800'>
            <div className='flex flex-wrap items-center justify-between gap-3'>
              <h3 className='text-xl font-semibold tracking-wide'>{t('teacher_attendance_list')}</h3>
                  ...
                  <PopoverAnchor asChild>
                    <div className='flex items-center gap-2 rounded-2xl border border-violet-200 bg-white p-2 shadow-sm'>
                      <SearchInput
                        ...
                        value={infoSearch?.teacher_attendance_code}
                        onKeyDown={(e) => {
                          if (e.key === 'Enter') {
                            getData({ offset: 0 });
                          }
                        ...
                        onChange={(value) => {
                          setInfoSearch({ ...infoSearch, teacher_attendance_code: value });
                        }}
                      />
...
export default AttendanceTeachersList;
