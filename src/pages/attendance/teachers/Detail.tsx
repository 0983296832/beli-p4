...
const AttendanceTeacherDetail = (props: Props) => {
  const { t } = useT();
  ...
  const getDetail = async () => {
    ...
    let params = {
      fields:
        'identity,timeline_id,attendance_code,attendance_time,classroom_id,status,teaching_program_id,teaching_program_detail_number,admin_assigned_id,subject_id,location_id,teacher_id,start_time,end_time,checkin_timestamp,checkout_timestamp,note,valid,user_ability,created_at,updated_at,teaching_program_detail,teaching_program'
    };
    try {
  ...
  const handleDelete = async () => {
    ...
    try {
      setLoading(false);
      const response: any = await attendanceServices.deleteTeacherAttendance(Number(id));
  ...
  const handleConfirm_Refuse = async (body: { status: 1 | 2; refuse_note?: string }) => {
    ...
    try {
      setLoading(false);
      const response: any = await attendanceServices.putTeacherAttendance(Number(id), body);
      Toast('success', response?.message);
      setIsShowConfirm(false);
        ...
        <div className='mx-auto max-w-6xl'>
          <div className='overflow-hidden rounded-3xl border border-violet-200 bg-white shadow-sm'>
            <div className='flex flex-wrap items-center justify-between gap-3 border-b border-violet-200 bg-violet-50 px-5 py-4'>
              <h3 className='text-xl font-semibold tracking-wide text-slate-800'>{t('teacher_attendance')}</h3>
              <div className='flex flex-wrap items-center gap-2'>
                ...
                {detail?.user_ability?.can_edit === 1 && (
                  <Button
                    ...
                    onClick={() => {
                      navigate('/attendance/teachers/edit/' + id);
                    }}
                  >
                    ...
                    <div className='flex w-full flex-col items-start gap-6 lg:flex-row lg:gap-8'>
                      <div className='w-full space-y-4 border-violet-100 lg:w-1/2 lg:border-r lg:pr-8'>
                        <div className='grid grid-cols-[150px_30px_1fr] gap-y-1 text-sm'>
                          ...
                          <div className='text-slate-400'>:</div>
                          <div className='text-slate-800'>{getUserInfo(detail?.teacher_id)?.display_name}</div>
                        </div>
                        <div className='grid grid-cols-[150px_30px_1fr] gap-y-1 text-sm'>
                          <div className='text-slate-500'>{t('code')}</div>
                          <div className='text-slate-400'>:</div>
                          <div className='text-slate-800'>{getUserInfo(detail?.teacher_id)?.username}</div>
                        </div>
                        <div className='grid grid-cols-[150px_30px_1fr] gap-y-1 text-sm'>
                      ...
                      <div className='min-w-0 flex-1'>
                        <div className='rounded-xl border border-violet-100 bg-violet-50/40 p-4 lg:pl-4'>
...
export default AttendanceTeacherDetail;
