...
const AttendanceStudentDetail = (props: Props) => {
  const { t } = useT();
  ...
  const handleDelete = async () => {
    ...
    try {
      setLoading(false);
      const response: any = await attendanceServices.deleteStudentAttendance(Number(id));
  ...
  const handleConfirm_Refuse = async (body: { status: 1 | 2; refuse_note?: string }) => {
    ...
    try {
      setLoading(false);
      const response: any = await attendanceServices.putStudentAttendance(Number(id), body);
      Toast('success', response?.message);
      setIsShowConfirm(false);
        ...
        <div className='mx-auto max-w-6xl'>
          <div className='overflow-hidden rounded-3xl border border-violet-200 bg-white shadow-sm'>
            <div className='flex flex-wrap items-center justify-between gap-3 border-b border-violet-200 bg-violet-50 px-5 py-4'>
              <h3 className='text-xl font-semibold tracking-wide text-slate-800'>{t('student_attendance')}</h3>
              <div className='flex flex-wrap items-center gap-2'>
                ...
                {detail?.user_ability?.can_edit === 1 && (
                  <Button
                    ...
                    onClick={() => {
                      navigate('/attendance/students/edit/' + id);
                    }}
                  >
                    ...
                    <div className='space-y-4'>
                      <div className='grid grid-cols-[250px_30px_1fr] gap-y-1 text-sm'>
                        ...
                        <div className='flex items-center gap-3'>
                          <p>{detail?.student_attendance_code}</p>
                          <div
                  ...
                  <div className='p-5 lg:p-6'>
                    <div className='space-y-4'>
                      <div className='grid grid-cols-[350px_30px_1fr] gap-y-1 text-sm'>
                        ...
                        <div className='text-slate-400'>:</div>
                        <div className='text-slate-800'>{detail?.teaching_program_detail?.lesson_name || t('not_updated')}</div>
                      </div>
                      ...
                      <div className='grid grid-cols-[350px_30px_1fr] gap-y-1 text-sm'>
                        ...
                        <a
                          ...
                          href={detail?.teaching_program_detail?.youtube_link || t('not_updated')}
                          target='_blank'
                          rel='noreferrer'
                        >
                          {detail?.teaching_program_detail?.youtube_link || t('not_updated')}
                        </a>
                      </div>
                      {currentUser?.user_job_title != USER_ROLE?.STUDENT && (
                        <div className='grid grid-cols-[350px_30px_1fr] gap-y-1 text-sm'>
                          ...
                          <a
                            className='font-medium text-violet-700 underline hover:text-violet-900'
                            href={detail?.teaching_program_detail?.drive_link || t('not_updated')}
                            target='_blank'
                            rel='noreferrer'
                          >
                            {detail?.teaching_program_detail?.drive_link || t('not_updated')}
                          </a>
                        </div>
                      )}
                      ...
                      <div className='grid grid-cols-[350px_30px_1fr] gap-y-1 text-sm'>
                        ...
                        <a
                          ...
                          rel='noreferrer'
                        >
                          {detail?.teaching_program_detail?.tiktok_link || t('not_updated')}
                        </a>
                      </div>
                        ...
                        <div
                          ...
                          dangerouslySetInnerHTML={{
                            __html: replaceNewlineWithBr(
                              detail?.teaching_program_detail?.lesson_content || t('not_updated')
                            )
                          }}
                        />
                      </div>
                  ...
                  <div className='flex flex-col gap-3 border-b border-violet-100 bg-violet-50/90 px-4 py-3 sm:flex-row sm:items-center sm:justify-between'>
                    ...
                    <div className='flex flex-col gap-2 sm:flex-row sm:items-center sm:gap-3'>
                      ...
                      <div className='flex h-10 items-center whitespace-nowrap rounded-xl border border-violet-200 bg-white px-4 text-sm text-slate-600 shadow-sm'>
                        {t('total')}:{' '}
                        <span className='ms-2 font-semibold text-violet-700'>{detail?.students?.length}</span>
                      </div>
                    </div>
                  </div>
                            ...
                            <div className='flex w-full max-w-full gap-4 border-violet-100 lg:w-max lg:border-r lg:pr-8'>
                              ...
                              <div className='min-w-0 flex-1 space-y-3'>
                                ...
                                <div className='grid grid-cols-[250px_30px_1fr] gap-y-1 text-sm'>
                                  ...
                                  <div className='text-slate-400'>:</div>
                                  <div className='text-slate-800'>{+student?.point || t('not_updated')}</div>
                                </div>
                                <div className='grid grid-cols-[250px_30px_1fr] gap-y-1 text-sm'>
                                  ...
                                  <div className='text-slate-800'>
                                    {' '}
                                    {attitudes?.find((s) => s?.value == student?.attitude)?.label || t('not_updated')}
                                  </div>
                                </div>
                                <div className='grid grid-cols-[250px_30px_1fr] gap-y-1 text-sm'>
                            ...
                            <div className='min-w-0 flex-1 lg:pl-4'>
                              <div className='rounded-xl border border-violet-100 bg-violet-50/40 p-4'>
                                ...
                                <div
                                  className='mt-1'
                                  dangerouslySetInnerHTML={{
                                    __html: replaceNewlineWithBr(student?.message || t('not_updated'))
                                  }}
                                />
                              </div>
                            </div>
        ...
        <div className='mx-auto flex max-w-6xl flex-wrap items-center justify-center gap-2 md:gap-4'>
          <div className='flex items-center gap-2 rounded-full bg-emerald-100 px-3 py-1.5 text-sm text-emerald-900'>
            ...
            <span className='rounded-full bg-white px-2 py-0.5 text-xs font-semibold text-emerald-800 shadow-sm'>
              {filter(detail?.students, (item) => [1].includes(item?.status))?.length}/{detail?.students?.length || 0} (
              {roundNumber(
                (filter(detail?.students, (item) => [1].includes(item?.status))?.length / detail?.students?.length) *
                  100 || 0
              )}
              %)
            </span>
          ...
          <div className='flex items-center gap-2 rounded-full bg-slate-200 px-3 py-1.5 text-sm text-slate-900'>
            ...
            <span className='rounded-full bg-white px-2 py-0.5 text-xs font-semibold text-slate-800 shadow-sm'>
              {filter(detail?.students, (item) => [2].includes(item?.status))?.length}/{detail?.students?.length || 0} (
              {roundNumber(
                (filter(detail?.students, (item) => [2].includes(item?.status))?.length / detail?.students?.length) *
                  100 || 0
              )}
              %)
            </span>
          ...
          <div className='flex items-center gap-2 rounded-full bg-rose-100 px-3 py-1.5 text-sm text-rose-900'>
            ...
            <span className='rounded-full bg-white px-2 py-0.5 text-xs font-semibold text-rose-800 shadow-sm'>
              {filter(detail?.students, (item) => item?.counted == 1)?.length}/{detail?.students?.length || 0} (
              {roundNumber(
                (filter(detail?.students, (item) => item?.counted == 1)?.length / detail?.students?.length) * 100 || 0
              )}
              %)
            </span>
          </div>
