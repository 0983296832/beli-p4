...
export const HistorySheet = ({ show, setShow, selectedId }: any) => {
  ...
  const getData = async () => {
    setLoading(true);
    try {
      const response: any = await attendanceServices.getStudentAttendancesLog(selectedId);
      setData(response);
    } catch (error) {
                          ...
                          <p
                            ...
                            onClick={() => {
                              if (log?.action == 4) {
                                ...
                                navigate(`/attendance/students/detail-compare/${selectedId}/log/${log?.id}`);
                              }
                            }}
                          >
...
const AttendanceStudentsList = (props: Props) => {
  const { t } = useT();
  const navigate = useNavigate();
  ...
  const handleDelete = async () => {
    ...
    try {
      setLoading(false);
      const response: any = await attendanceServices.deleteStudentAttendance(Number(selectedId));
  ...
  const getData = async (filters?: any) => {
    let params: any = {
      fields:
        'identity,attendance_id,timeline_id,student_attendance_code,attendance_time,classroom_id,teaching_program_id,teaching_program_detail_number,admin_assigned_id,subject_id,location_id,status,valid,created_at,updated_at,user_ability,students,teaching_program_detail,teaching_program',
      limit,
      offset,
      filterings: {}
    };
    if (infoSearch?.student_attendance_code) {
      params.filterings['student_attendance_code:contain'] = infoSearch?.student_attendance_code;
    }
    ...
    setLoading(true);
    try {
      const response: any = await attendanceServices.getStudentAttendances(params);
  ...
  const exportData = async () => {
    ...
    setLoading(true);
    try {
      const response: any = await attendanceServices.getStudentAttendanceExport(params);
      downloadFileFromBlob(response, 'danh_sach_diem_danh_hoc_sinh_vhs.xlsx');
    } catch (error) {
      console.error('Error fetching data:', error);
    } finally {
      setLoading(false);
        ...
        <div className='w-full overflow-hidden rounded-3xl border border-violet-200 bg-white shadow-sm'>
          <div className='border-b border-violet-200 bg-violet-50 px-5 py-5 text-slate-800'>
            <div className='flex flex-wrap items-center justify-between gap-3'>
              <h3 className='text-xl font-semibold tracking-wide'>{t('student_attendance_list')}</h3>
                  ...
                  <PopoverAnchor asChild>
                    <div className='flex items-center gap-2 rounded-2xl border border-violet-200 bg-white p-2 shadow-sm'>
                      <SearchInput
                        ...
                        value={infoSearch?.student_attendance_code}
                        onKeyDown={(e) => {
                          if (e.key === 'Enter') {
                            getData({ offset: 0 });
                          }
                        ...
                        onChange={(value) => {
                          setInfoSearch({ ...infoSearch, student_attendance_code: value });
                        }}
                      />
                    ...
                    <div className='grid grid-cols-2 gap-2'>
                      {/* <div className='col-span-2'>
                        <DateRangeInput
                          ...
                          startTimestamp={infoSearch?.attendance_start_time}
                          endTimestamp={infoSearch?.attendance_end_time}
                          onChange={(start, end) => {
                            setInfoSearch({ ...infoSearch, attendance_start_time: start, attendance_end_time: end });
                          }}
                        />
                        <DateInput
                          label={t('attendance_date')}
            ...
            <thead>
              <tr>
                ...
                <th scope='col' className='bg-violet-100/70 text-violet-900'>
                  {t('attendance_session_code')}-{t('students')}
                </th>
                ...
                <th scope='col' className='bg-emerald-100/70 text-emerald-900'>
                  {t('period_order_in_program')}
                </th>
                <th scope='col' className='bg-cyan-100/70 text-cyan-900'>{t('attendance_statistics')}</th>
                <th scope='col' className='bg-violet-50 text-violet-900'>{t('subject')}</th>
            ...
            <tbody className='bg-white'>
              {data?.length > 0 ? (
                data.map((attendant: any, index: number) => {
                  const attend_class = `${filter(attendant?.students, (item) => [1].includes(item?.status))?.length}/
                                ${attendant?.students?.length || 0} (
                                ${roundNumber(
                                  (filter(attendant?.students, (item) => [1].includes(item?.status))?.length /
                                    attendant?.students?.length) *
                                    100 || 0
                                )}
                                %)`;
                  const absent = `${filter(attendant?.students, (item) => [2].includes(item?.status))?.length}/
              ${attendant?.students?.length || 0} (
              ${roundNumber(
                (filter(attendant?.students, (item) => [2].includes(item?.status))?.length /
                  attendant?.students?.length) *
                  100 || 0
              )}
              %)`;
                  const exclue_lesson = `${filter(attendant?.students, (item) => item?.counted == 1)?.length}/${attendant?.students?.length || 0} (
              ${roundNumber((filter(attendant?.students, (item) => item?.counted == 1)?.length / attendant?.students?.length) * 100 || 0)}
              %)`;
                  const no_exclue_lesson = `${filter(attendant?.students, (item) => item?.counted !== 1)?.length}/${attendant?.students?.length || 0} (
              ${roundNumber((filter(attendant?.students, (item) => item?.counted !== 1)?.length / attendant?.students?.length) * 100 || 0)}
              %)`;
                  return (
                    <tr
                    ...
                    >
                      ...
                      <td>
                        <div className='flex items-center gap-2'>
                          <div className='text-start'>
                            <Link
                              to={`/attendance/students/detail/${attendant?.id}`}
                              className='font-medium text-violet-700 hover:underline'
                            >
                              {attendant?.student_attendance_code}
                            </Link>
                            <div
                      ...
                      <td className='space-y-1 text-start'>
                        <div>{`${attendant?.teaching_program_detail_number}/${attendant?.teaching_program?.lesson_count} (${roundNumber((attendant?.teaching_program_detail_number / attendant?.teaching_program?.lesson_count) * 100)}%)`}</div>
                        <div>{attendant?.teaching_program_detail?.lesson_name}</div>
                        <div>{attendant?.teaching_program_detail?.teaching_program_detail_code}</div>
                      </td>
                      <td className='space-y-1 text-start whitespace-nowrap'>
                        <div>
                          {t('attend_class')}: {attend_class}
                        </div>
                        <div>
                          {t('absent')}: {absent}
                        </div>
                        <div>
                          {t('_exclude_lesson')}: {exclue_lesson}
                        </div>
                        <div>
                          {t('_no_deduction_class')}: {no_exclue_lesson}
                        </div>
                      </td>
                      <td>
                      ...
                      <td className='whitespace-nowrap w-[1%]'>
                        <div className='flex items-center justify-center gap-1'>
                          {attendant?.user_ability?.can_edit === 1 && (
                            <Link to={`/attendance/students/edit/${attendant?.id}`}>
                              <img src={EDIT_02} alt='' className='size-4' />
                            </Link>
                          )}
