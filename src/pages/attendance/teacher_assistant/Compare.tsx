...
const Compare = (props: Props) => {
  ...
  const getDataCompare = async () => {
    setLoading(true);
    try {
      const response: any = await attendanceServices.getTAAttendancesLogCompare(Number(id), Number(log_id));
                    ...
                    <div className='grid grid-cols-2 gap-x-6 gap-y-4'>
                      <div className='grid grid-cols-2 gap-6'>
                        <div className='col-span-2'>
                          <p className={`${checkKeyDiff(`ta_id`)} text-sm mb-2`}>{t('teaching_assistant')}</p>
                          <UsersSelect
                            disabled
