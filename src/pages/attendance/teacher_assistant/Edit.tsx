...
const AttendanceTeacherAssistantEdit = (props: Props) => {
  ...
  const [formData, setFormData] = useState<any>({
    ...
    ta_id: currentUser?.user_job_title == USER_ROLE.TA ? currentUser?.user_id : null
  });
  ...
  const getDetail = async () => {
    ...
    try {
      ...
      setFormData({
        ...
        // },
        note: data?.note,
        teaching_program_detail: data?.teaching_program_detail
      });
    } catch (error) {
    } finally {
      setLoading(false);
  ...
  const onUpdate = async () => {
    // if (!formData.checkin_timestamp) {
    //   return Toast('warning', t('checkin_photo_required'));
    // }
    // if (!formData.checkout_timestamp) {
    //   return Toast('warning', t('checkout_photo_required'));
    // }
    setLoading(true);
    try {
      if (Number(id)) {
        ...
        const res: any = await attendanceServices.putTAAttendance(Number(id), {
        ...
        if (isDuplicate) {
          ...
          delete body?.teaching_program_detail;
          delete body.schedules;
          delete body.start_date;
          delete body.end_date;
          const res: any = await attendanceServices.postTAAttendance(body);
          Toast('success', res?.message);
          setFormData({});
          setErrors({});
          setLoading(false);
  ...
  const checkDuplicate = async () => {
    setLoading(true);
    try {
      const res: any = await attendanceServices.postTAAttendanceDuplicate({ timeline_id: formData?.timeline_id });
      ...
      <AlertDialog open={isShowDuplicate} onOpenChange={setIsShowDuplicate}>
        <AlertDialogContent className='max-w-[350px] rounded-lg'>
          ...
          <AlertDialogFooter className=' flex justify-center items-center flex-col gap-4'>
            <Link
              to={`/attendance/teachers_assistant/detail/${duplicateData?.attendance_id}`}
                  ...
                  <div className='full'>
                    <PeriodSelect
                      ...
                      onChange={(value) => {
                        setFormData({
                          ...formData,
                          timeline_id: value?.id,
                          teaching_program_detail: value?.teaching_program_detail
                        });
                  ...
                  <div className='p-6 card-body'>
                    <div className='grid grid-cols-2 gap-x-8 gap-y-4'>
                      <TextInput
                        ...
                        disabled
                        value={formData?.teaching_program_detail?.teaching_program_detail_code}
                      />
                      <TextInput
                        label={t('period_order_in_program')}
                        disabled
                        value={formData?.teaching_program_detail?.lesson_number}
                      />
                      <div className='col-span-2'>
                        <TextInput
                          label={t('lesson_title')}
                          disabled
                          value={formData?.teaching_program_detail?.lesson_name}
                        />
                      </div>
                      <div className='col-span-2'>
                        <TextAreaInput
                          ...
                          rows={6}
                          value={formData?.teaching_program_detail?.lesson_content}
                          disabled
                        />
                      </div>
                      <div>
                        <TextInput
                          ...
                          value={formData?.teaching_program_detail?.youtube_link}
                          disabled
                          className='text-primary-blue-500 underline disabled:text-primary-blue-500'
                        />
                      </div>
                      <div>
                        <TextInput
                          label={t('lesson_content_drive')}
                          value={formData?.teaching_program_detail?.drive_link}
                          disabled
                          className='text-primary-blue-500 underline disabled:text-primary-blue-500'
                        />
                      </div>
                      ...
                      <div>
                        <TextInput
                          label={t('lesson_content_tiktok')}
                          value={formData?.teaching_program_detail?.tiktok_link}
                          disabled
                          className='text-primary-blue-500 underline disabled:text-primary-blue-500'
                        />
                      </div>
                        ...
                        {currentUser?.user_job_title != USER_ROLE.TA && (
                          <div className='col-span-2'>
                            <UsersSelect
                              ...
                              error={errors?.ta_id}
                              onChange={(val) => {
                                setFormData({ ...formData, ta_id: val?.id });
...
export default AttendanceTeacherAssistantEdit;
