...
const ReportSession = (props: Props) => {
  ...
  const handleDelete = async () => {
    ...
    try {
      setLoading(false);
      const response: any = await exerciseServices.deleteStudentExercise(selectedId);
  ...
  const getData = async (filters?: any) => {
    let params: any = {
      fields:
        'exam_code,exam_name,identity,exercise_id,exercise,exam_code,filters,created_by_type,creator,valid,start_time,end_time,mode,user_ability',
      limit,
      offset,
      filterings: {
    ...
    setLoading(true);
    try {
      const response: any = await reportExamServices.getExamSessionsReport(Number(id), params);
  ...
  const exportData = async (filters?: any) => {
    ...
    setLoading(true);
    try {
      const response: any = await reportExamServices.getExamExport(Number(id), params);
  ...
  return (
    <>
          ...
          <div className='p-4 bg-white'>
            <div className='flex justify-between'>
              <Popover open={openFilter} onOpenChange={setOpenFilter}>
                ...
                <PopoverContent className='w-[730px]' align='start' onInteractOutside={(e) => e.preventDefault()}>
                  ...
                  <tr key={index}>
                    ...
                    <td className=''>
                      <div className='flex items-center justify-center gap-3'>
                        <TooltipUi description={t('download')}>
                          <Button
                            variant='secondary'
                        ...
                        {!!item?.user_ability?.can_delete && currentUser?.user_job_title !== USER_ROLE?.STUDENT && (
                          <TooltipUi description={t('delete')}>
                            ...
                            >
                              <img src={DELETE_04} className='cursor-pointer' alt='' />
                            </Button>
                          </TooltipUi>
                        )}
                      </div>
      ...
      <div className='fixed bottom-0 left-[70px] right-0 z-[49] px-4 py-2 bg-primary-neutral-100 border-t border-primary-neutral-300'>
        <Pagination
          limit={limit}
          offset={offset}
