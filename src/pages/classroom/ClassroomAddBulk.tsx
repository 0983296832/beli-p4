...
const ClassroomAddBulk = (props: Props) => {
  const { t } = useT();
  const { syncClassroomsStore } = useRootStore();
  ...
  const onValidateFile = async () => {
    try {
      ...
      const res: any = await classroomServices.postClassroomImportForm(formData);
  ...
  const onUploadFile = async () => {
    try {
      setLoading(true);
      const res_upload: any = await classroomServices.postClassroomImport({ data: uploadData });
          ...
          <div className='border-b border-violet-200 bg-violet-50 px-5 py-4'>
            <div className='flex flex-wrap items-center justify-between gap-3'>
              <div className='min-w-0'>
                <p className='text-xs font-medium uppercase tracking-wide text-violet-600'>{t('bulk_add_classes')}</p>
                <h3 className='mt-0.5 truncate text-xl font-semibold text-slate-800'>{t('add_multiple_classes')}</h3>
              </div>
              <Button
                type='button'
                variant='outline'
...
export default ClassroomAddBulk;
