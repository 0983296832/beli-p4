...
const TeacherDetail = (props: Props) => {
  ...
  const handleDelete = async () => {
    ...
    try {
      setLoading(false);
      const response: any = await teacherServices.deleteTeacher(Number(id));
      Toast('success', response?.message);
      setIsShowConfirm(false);
      navigate('/user/teacher/list');
    } catch (error) {
      setLoading(false);
      setIsShowConfirm(false);
    }
        ...
        <div className='overflow-hidden rounded-3xl border border-violet-200 bg-white shadow-sm'>
          <div className='flex flex-wrap items-center justify-between gap-3 border-b border-violet-200 bg-violet-50 px-5 py-4'>
            <div className='min-w-0'>
              <p className='text-xs font-medium uppercase tracking-wide text-violet-600'>{t('teacher_detail')}</p>
                ...
                <section className='rounded-2xl border border-sky-200 bg-sky-50/30'>
                  ...
                  <div className='bg-white p-4 text-sm text-slate-700 lg:p-5'>
                    <div className='flex min-h-[100px] flex-wrap items-center gap-2 rounded-xl border border-sky-100 bg-sky-50/40 p-4'>
                      <p>{convertTimestampToString(detail?.latest_teacher_attendance?.attendance_time)}</p>
                      <p>{getClassroomInfo(detail?.latest_teacher_attendance?.classroom_id)?.classroom_name}</p>
                    </div>
                  </div>
                </section>
