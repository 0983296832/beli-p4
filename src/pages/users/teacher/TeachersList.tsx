...
const TeachersList = (props: Props) => {
  ...
  const handleDelete = async () => {
    ...
    try {
      setLoading(false);
      const response: any = await teacherServices.deleteTeacher(Number(selectedId));
  ...
  const getData = async (filters?: any) => {
    ...
    setLoading(true);
    try {
      const response: any = await teacherServices.getTeachers(params);
  ...
  const onUpdateDescription = async (id: number, description: string) => {
    setLoading(true);
    try {
      const res: any = await teacherServices.putTeacher(Number(id), { description });
      getData();
      ...
      <div className='space-y-4 pb-24'>
        ...
        <div className='w-full overflow-hidden rounded-3xl border border-violet-200 bg-white shadow-sm'>
          <div className='border-b border-violet-200 bg-violet-50 px-5 py-5 text-slate-800'>
            <h3 className='text-xl font-semibold tracking-wide'>{t('teacher_list')}</h3>
          </div>
              ...
              <div className='flex flex-wrap items-center gap-2'>
                ...
                {!!permissions.CREATE_TEACHER && (
                  <Button
                    ...
                    onClick={() => navigate('/user/teacher/add')}
                  >
                ...
                {/* <DropdownMenu>
                  ...
                  <DropdownMenuContent>
                    <DropdownMenuItem>
                      <Link to='/user/teacher/add'>{t('add_single')}</Link>
                    </DropdownMenuItem>
                    <DropdownMenuItem>
                      <Link to='/user/teacher/add_bulk'>{t('add_bulk')}</Link>
                    </DropdownMenuItem>
                  </DropdownMenuContent>
                </DropdownMenu> */}
              </div>
