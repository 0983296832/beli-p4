  ...
  const onUpdate = async () => {
    ...
    try {
      if (id) {
        ...
        delete body.age;
        const res: any = await teacherServices.putTeacher(Number(id), body);
        ...
        delete body.username;
        delete body.age;
        const res: any = await teacherServices.postTeacher(body);
        Toast('success', res?.message);
        setFormData({});
        setErrors({});
        setLoading(false);
      ...
      <div className='w-full shadow-lg card bg-primary-neutral-50'>
        <div className='card-header text-neutral-50 bg-primary-blue-500'>
          <h3 className='text-base font-semibold leading-[100%]'>
            {id ? t('edit_teacher') : t('add_teacher_account')}
          </h3>
        </div>
        <div className='p-6 card-body'>
          ...
          {id && (
            <div className='flex justify-center gap-6 mt-6'>
              ...
              <Button variant='default' className='px-14' onClick={onUpdate} disabled={loading}>
                {t('update')}
              </Button>
            </div>
          )}
