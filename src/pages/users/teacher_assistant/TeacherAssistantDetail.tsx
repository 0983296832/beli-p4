...
const TeacherAssistantDetail = (props: Props) => {
  const { t } = useT();
  ...
  const getDetail = async () => {
    ...
    try {
      const res: any = await teacherAssistantServices.getTeacherAssistant(Number(id), params);
      setDetail(res);
    } catch (error) {
    } finally {
      setLoading(false);
  ...
  const handleDelete = async () => {
    ...
    try {
      setLoading(false);
      const response: any = await teacherAssistantServices.deleteTeacherAssistant(Number(id));
        ...
        <div className='overflow-hidden rounded-3xl border border-violet-200 bg-white shadow-sm'>
          <div className='flex flex-wrap items-center justify-between gap-3 border-b border-violet-200 bg-violet-50 px-5 py-4'>
            <div className='min-w-0'>
              <p className='text-xs font-medium uppercase tracking-wide text-violet-600'>{t('ta_detail')}</p>
            ...
            <div className='flex flex-wrap items-center gap-2'>
              ...
              {detail?.user_ability?.can_edit == 1 && (
                <Button
                  ...
                  onClick={() => {
                    navigate('/user/teacher_assistant/edit/' + id);
                  }}
                >
              ...
              <div className='min-w-0 flex-1 space-y-4'>
                  ...
                  <section className='rounded-2xl border border-violet-200 bg-white lg:col-span-7'>
                    ...
                    <div className='divide-y divide-violet-100 p-4 text-sm'>
                      <div className='grid grid-cols-1 gap-1 py-3 first:pt-0 sm:grid-cols-[140px_1fr] sm:gap-3'>
                        <span className='font-medium text-slate-500'>{t('teacher_name')}</span>
                  ...
                  <section className='rounded-2xl border border-emerald-200 bg-white lg:col-span-5'>
                    ...
                    <div className='divide-y divide-emerald-100 p-4 text-sm'>
                      <div className='flex items-center justify-between gap-3 py-3 first:pt-0'>
                        <span className='font-medium text-slate-600'>{t('students')}</span>
                        <button
                          type='button'
                        ...
                        >
                          {detail?.manager_statistic?.students}
                        </button>
                      </div>
                      ...
                      <div className='flex items-center justify-between gap-3 py-3'>
                        <span className='font-medium text-slate-600'>{t('teaching_program')}</span>
                        <button
                          type='button'
                          className='min-w-[2rem] rounded-lg bg-emerald-50 px-2 py-1 text-right font-semibold text-violet-700 underline decoration-violet-300 underline-offset-2 hover:bg-emerald-100'
                          onClick={() => {
                        ...
                        >
                          {detail?.manager_statistic?.teaching_programs}
                        </button>
                      </div>
                      <div className='flex items-center justify-between gap-3 py-3'>
                        <span className='font-medium text-slate-600'>{t('curriculum')}</span>
                        <button
                          type='button'
                          className='min-w-[2rem] rounded-lg bg-emerald-50 px-2 py-1 text-right font-semibold text-violet-700 underline decoration-violet-300 underline-offset-2 hover:bg-emerald-100'
                          onClick={() => {
                        ...
                        >
                          {detail?.manager_statistic?.learning_programs}
                        </button>
                      </div>
                    </div>
                  </section>
                ...
                <section className='rounded-2xl border border-sky-200 bg-sky-50/30'>
                  <div className='flex items-center gap-2 border-b border-sky-100 bg-white/80 px-4 py-3'>
                    ...
                    <h2 className='text-sm font-semibold text-sky-900'>{t('last_teaching_attendance_date')}</h2>
                  </div>
                  <div className='bg-white p-4 text-sm text-slate-700 lg:p-5'>
                    <div className='flex min-h-[100px] flex-wrap items-center gap-2 rounded-xl border border-sky-100 bg-sky-50/40 p-4'>
                      <p>{convertTimestampToString(detail?.latest_ta_attendance?.attendance_time)}</p>
                      <p>{getClassroomInfo(detail?.latest_ta_attendance?.classroom_id)?.classroom_name}</p>
                    </div>
                  </div>
                </section>
