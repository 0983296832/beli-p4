...
import { CAMERA, NO_AVATAR, NO_IMAGE } from '@lib/ImageHelper';
...
const TeacherAssistantAdd = (props: Props) => {
  const { id } = useParams();
  ...
  const onUpdate = async () => {
    ...
    try {
      if (id) {
        ...
        delete body.age;
        const res: any = await teacherAssistantServices.putTeacherAssistant(Number(id), body);
        ...
        delete body.username;
        delete body.age;
        const res: any = await teacherAssistantServices.postTeacherAssistant(body);
        Toast('success', res?.message);
        setFormData({});
        setErrors({});
        setLoading(false);
      ...
      <div className='w-full shadow-lg card bg-primary-neutral-50'>
        <div className=' text-neutral-50 bg-primary-blue-500 card-header'>
          <h3 className='text-base font-semibold leading-[100%]'>
            {id ? t('edit_ta') : t('add_teaching_assistant_account')}
          </h3>
        </div>
              ...
              <div className='w-full border rounded-lg border-primary-neutral-300 card'>
                ...
                <div className='p-6 card-body'>
                  <div className='grid grid-cols-2 mb-4 gap-x-9 gap-y-4'>
                    <TextInput
                      label={t('teaching_assistant_name')}
                      required
...
export default TeacherAssistantAdd;
