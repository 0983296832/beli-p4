...
const TeacherAssistantList = (props: Props) => {
  ...
  const handleDelete = async () => {
    ...
    try {
      setLoading(false);
      const response: any = await teacherAssistantServices.deleteTeacherAssistant(Number(selectedId));
  ...
  const getData = async (filters?: any) => {
    ...
    setLoading(true);
    try {
      const response: any = await teacherAssistantServices.getTeacherAssistants(params);
  ...
  const exportData = async () => {
    ...
    setLoading(true);
    try {
      const response: any = await teacherAssistantServices.getTeacherAssistantExport(params);
      downloadFileFromBlob(response, 'danh_sach_tro_giang_beli.xlsx');
    } catch (error) {
      console.error('Error fetching data:', error);
    } finally {
      setLoading(false);
  ...
  const onUpdateDescription = async (id: number, description: string) => {
    setLoading(true);
    try {
      const res: any = await teacherAssistantServices.putTeacherAssistant(Number(id), { description });
      getData();
      ...
      <div className='space-y-4 pb-24'>
        ...
        <div className='w-full overflow-hidden rounded-3xl border border-violet-200 bg-white shadow-sm'>
          <div className='border-b border-violet-200 bg-violet-50 px-5 py-5 text-slate-800'>
            <h3 className='text-xl font-semibold tracking-wide'>{t('teaching_assistant_list')}</h3>
          </div>
            ...
            <div className='flex flex-wrap items-center justify-between gap-3'>
              ...
              <div className='flex flex-wrap items-center gap-2'>
                ...
                {/* <DropdownMenu>
                  ...
                  <DropdownMenuContent>
                    <DropdownMenuItem>
                      <Link to='/user/teacher_assistant/add'>{t('add_single')}</Link>
                    </DropdownMenuItem>
                    <DropdownMenuItem>
                      <Link to='/user/teacher_assistant/add_bulk'>{t('add_bulk')}</Link>
                    </DropdownMenuItem>
                  </DropdownMenuContent>
                </DropdownMenu> */}
              </div>
                ...
                <thead>
                  <tr>
                    ...
                    <th scope='col' rowSpan={2} className='bg-violet-100/70 text-violet-900'>
                      {t('teaching_assistant_code_name')}
                    </th>
                    <th scope='col' rowSpan={2} className='bg-violet-100/70 text-violet-900'>
                      {t('date_of_birth')}
                      <br />
                    ...
                    data.map((user: any, index: number) => (
                      ...
                      >
                        ...
                        <td className='bg-violet-50/60'>
                          <div className='flex items-center gap-2'>
                            ...
                            <div className='text-start'>
                              ...
                              <Link to={`/user/teacher_assistant/detail/${user.id}`} className='text-primary-blue-500'>
                                {user.display_name}
                              </Link>
                        ...
                        <td className='bg-sky-50/70'>
                          <p>{convertTimestampToString(user?.latest_ta_attendance?.attendance_time)}</p>
                          <p>{getClassroomInfo(user?.latest_ta_attendance?.classroom_id)?.classroom_name}</p>
                        </td>
                        ...
                        <td className='whitespace-nowrap w-[1%]'>
                          <div className='flex items-center justify-center gap-1'>
                            {!!user?.user_ability?.can_edit && (
                              <Link to={`/user/teacher_assistant/edit/${user.id}`}>
                                <img src={EDIT_02} alt='' />
                              </Link>
                            )}
...
export default TeacherAssistantList;
