...
const AdministratorDetail = (props: Props) => {
  const { t } = useT();
  ...
  const getDetail = async () => {
    ...
    let params = {
      fields:
        'user_id,province_name,district_name,ward_name,age,username,user_job_title,description,email,phone,display_name,avatar,birthday,gender,verify_code,verify_status,status,valid,role_id,created_at,updated_at,address,working_status,province_id,district_id,ward_id,manager_statistic,user_ability'
    };
    try {
  ...
  const handleDelete = async () => {
    ...
    try {
      setLoading(false);
      const response: any = await adminServices.deleteAdmin(Number(id));
        ...
        <div className='overflow-hidden rounded-3xl border border-violet-200 bg-white shadow-sm'>
          <div className='flex flex-wrap items-center justify-between gap-3 border-b border-violet-200 bg-violet-50 px-5 py-4'>
            <div className='min-w-0'>
              <p className='text-xs font-medium uppercase tracking-wide text-violet-600'>{t('admin_detail')}</p>
            ...
            <div className='flex flex-wrap items-center gap-2'>
              ...
              {detail?.user_ability?.can_edit == 1 && (
                <Button
                  ...
                  onClick={() => {
                    navigate('/user/administrator/edit/' + id);
                  }}
                >
                ...
                <div className='grid gap-4 lg:grid-cols-12'>
                  <section className='rounded-2xl border border-violet-200 bg-white lg:col-span-7'>
                    <div className='flex items-center gap-2 border-b border-violet-100 bg-violet-50/80 px-4 py-3'>
                      ...
                      <h2 className='text-sm font-semibold text-violet-900'>{t('admin_detail')}</h2>
                    </div>
                        ...
                        <button
                          ...
                          onClick={() => {
                            setStatisticInfo({
                              ...
                              extraFilter: {
                                filterings: {
                                  ['admin_id:eq']: detail?.id
                                }
                              }
                            });
                          }}
                        >
                          {detail?.manager_statistic?.teachers}
                        </button>
                      </div>
                      <div className='flex items-center justify-between gap-3 py-3'>
                        <span className='font-medium text-slate-600'>{t('teaching_assistant')}</span>
                        <button
                          type='button'
                          className='min-w-[2rem] rounded-lg bg-emerald-50 px-2 py-1 text-right font-semibold text-violet-700 underline decoration-violet-300 underline-offset-2 hover:bg-emerald-100'
                          onClick={() => {
