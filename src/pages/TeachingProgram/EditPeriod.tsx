...
const EditPeriod = ({ show, setShow, id, periodsNumber: lesson_number, getData, teachingProgramId }: Props) => {
  ...
  return (
    <AlertDialog open={show} onOpenChange={setShow}>
      <AlertDialogContent className='rounded-3xl border border-violet-200 gap-0 p-0 max-w-[70%] overflow-hidden bg-white shadow-sm'>
        <AlertDialogHeader className='flex flex-row justify-between px-6 py-4 space-y-0 border-b border-violet-200 bg-violet-50 text-slate-800'>
          <div className='text-lg font-semibold'>{t(id ? 'add_lesson_content' : 'edit_lesson_content')}</div>
        </AlertDialogHeader>
        ...
        <div className='max-h-[calc(100vh-200px)] overflow-y-auto p-6'>
          <div className='space-y-4'>
            ...
            <section className='rounded-2xl border border-amber-200 bg-amber-50/30 p-4'>
              <div className='grid grid-cols-2 gap-x-9 gap-y-4'>
                <TextInput
            ...
            <section className='rounded-2xl border border-fuchsia-200 bg-fuchsia-50/30 p-4'>
              <Upload
                accept={{ 'image/*': [] }}
                maxFiles={5}
                useService
