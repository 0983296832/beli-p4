...
const Practice = (props: Props) => {
  ...
  const getDetail = async () => {
    ...
    try {
      ...
      setExamData({ ..._examData, exercise_code: res?.data?.exam_code, exercise_name: res?.data?.exam_name });
  ...
  const handleAccessExam = async () => {
    try {
      setLoading(true);
      await exerciseServices.postExamAccess(Number(id));
      setLoading(false);
    } catch (error) {
      setLoading(false);
    }
  ...
  return (
    <>
      <Helmet>
        <title>{examDetail ? `Bài tập ${examDetail.exam_code} | ${examDetail.exam_name}` : 'Đang tải...'}</title>
        {examDetail && (
          <meta name='description' content={`Chi tiết bài tập ${examDetail.exam_code}: ${examDetail.exam_name}`} />
        )}
      </Helmet>
      <div
        ...
        {!loading && (
          <>
            {isReady ? (
              <>
                {setting?.mode == 3 ? (
                  <Exam examData={examData} setting={setting} examId={Number(id)} />
                ) : (
                  <Training examData={examData} setting={setting} examId={Number(id)} />
                )}
              </>
            ) : (
