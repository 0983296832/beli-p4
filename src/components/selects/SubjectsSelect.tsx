...
const SubjectsSelect = <IsMulti extends boolean = false>({
  label,
  value,
  onChange,
  required,
  ...
  const { t } = useT();
  const [options, setOptions] = useState<SubjectType[]>([]);
  const [groupOptions, setGroupOptions] = useState<{ id: number; label: string; options: SubjectType[] }[]>([]);
  const [loading, setLoading] = useState(false);

  const getSubjects = async () => {
    setLoading(true);
    const res: any = await subjectServices.getSharedSubjects();
    if (isMulti) {
      setGroupOptions([
        {
          label: t('select_all'),
          ...
          options: res?.data?.map((i: any) => ({
            ...i,
            label: `${i?.subject_code} - ${i?.subject_name}`
          }))
        }
      ]);
    }
    setOptions(res?.data?.map((i: any) => ({ ...i, label: `${i?.subject_code} - ${i?.subject_name}` })));
    setLoading(false);
  };
  ...
  const CustomMultiValue = (props: any) => {
    const subject: SubjectType = props.data;
  ...
  const formatGroupLabel = (data: { label: string; options: any[] }) => {
    ...
    const toggleGroup = () => {
      ...
      if (isGroupChecked) {
        ...
        (onChange as (value: SubjectType[]) => void)(newSelected);
      } else {
        ...
        (onChange as (value: SubjectType[]) => void)(merged);
      }
    };
    return (
      <div
    ...
    return (
      <div {...innerProps} className='flex items-center p-2 gap-2 hover:bg-[#C4DBEF] cursor-pointer rounded-sm'>
        <div className='flex items-center gap-3'>
          ...
          <img src={data?.subject_avatar} className='w-8 h-8 min-w-8 min-h-8 rounded-full object-cover' />
        </div>
  ...
  return (
    <SelectInput
      ...
      keyValue='id'
      keyLabel='label'
      placeholder={placeholder || t('choose_subject')}
      className={className}
